...
public class NextThroughNotesTest {
    ...
    @Rule
    public ActivityScenarioRule<MainActivity> mMainActivityScenarioRule
    ...
    public void nextThroughNotes(){
        ...
        for(int index = 0; index<notes.size(); ++index) {
            ...
            if(index < notes.size()-1)
                onView(allOf(withId(R.id.acton_next), isEnabled())).perform(click());
        }

        onView(withId(R.id.acton_next)).check(matches(not(isEnabled())));
        pressBack();
    }
}